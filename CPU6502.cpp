...
void CPU6502::reset() noexcept
{
	...
	m_status = 0 | toUType(FLAGS6502::U);
	m_addrRel = 0;
	m_addrAbs = 0;
	m_fetched = 0;
	m_cycles = 8;
...
uint8_t CPU6502::IZX()
{
	...
	uint16_t low = read(static_cast<uint16_t>(t + m_x) & 0x00FF);
	uint16_t high = read(static_cast<uint16_t>(t + m_x + 1) & 0x00FF);
...
uint8_t CPU6502::PHP()
{
	write(0x0100 + m_stkp, m_status | toUType(FLAGS6502::B) | toUType(FLAGS6502::U));
	setFlag(FLAGS6502::B, 0);
...
uint8_t CPU6502::BIT()
{
	...
	setFlag(FLAGS6502::N, m_fetched & (1 << seven_bits));
	setFlag(FLAGS6502::V, m_fetched & (1 << six_bits));
	return 0;
}
...
uint8_t CPU6502::ADC()
{
	...
	m_temp = static_cast<uint16_t>(m_a + m_fetched + getFlag(FLAGS6502::C));

	setFlag(FLAGS6502::C, m_temp > 255);
	setZifZero(m_temp & 0x00FF);

	auto cond{ (~(static_cast<uint16_t>(m_a) ^ static_cast<uint16_t>(m_fetched)) & 
		(static_cast<uint16_t>(m_a) ^ static_cast<uint16_t>(m_temp))) & 0x0080 };

	setFlag(FLAGS6502::V, static_cast<uint16_t>(cond));
	setNifMsbSet(m_temp);
...
{
	fetch();
	auto value = static_cast<uint16_t>(m_fetched ^ 0x00FF);
	m_temp = static_cast<uint16_t>(m_a + value + getFlag(FLAGS6502::C));

	setFlag(FLAGS6502::C, m_temp & 0xFF00);
	setZifZero(m_temp & 0x00FF);
	setFlag(FLAGS6502::V, (m_temp ^ static_cast<uint16_t>(m_a)) & (m_temp ^ value) & 0x0080);
	setNifMsbSet(m_temp);
...
void CPU6502::fetchAndCompare(uint16_t var) {
	...
	setFlag(FLAGS6502::C, var >= m_fetched);
	setZifZero(m_temp & 0x00FF);
	setNifMsbSet(m_temp);
}
...
uint8_t CPU6502::ASL()
{
	shift(
		...
		[=](){ setFlag(FLAGS6502::C, (m_temp & 0xFF00) > 0); }
	);
...
uint8_t CPU6502::LSR()
{
	shift(
		...
		[=]() { setFlag(FLAGS6502::C, m_fetched & 0x0001); }
	);
...
uint8_t CPU6502::ROL()
{
	shift(
		...
		[=]() { m_temp = static_cast<uint16_t>(m_fetched << one_bit) | getFlag(FLAGS6502::C); },
...
uint8_t CPU6502::ROR()
{
	shift(
		...
		[=]() { m_temp = static_cast<uint16_t>(getFlag(FLAGS6502::C) << seven_bits) | (m_fetched >> one_bit); },

		[=]() { setFlag(FLAGS6502::C, m_fetched & 0x01); }
	);
...
uint8_t CPU6502::RTI()
{
	++m_stkp;
	m_status = read(0x0100 + m_stkp);
	m_status &= ~toUType(FLAGS6502::B);
	m_status &= ~toUType(FLAGS6502::U);
